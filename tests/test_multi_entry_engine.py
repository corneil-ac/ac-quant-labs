...
def _candles(values):
    ...
    return pd.DataFrame({
        ...
        "high": values + .2, "low": values - .2, "close": values,
    })


def _market(direction):
    if direction == "BUY":
        h1, m15 = _candles(np.linspace(100, 200, 220)), _candles(np.linspace(150, 200, 70))
        m15.loc[69, ["open", "high", "low", "close"]] = [198, 202, 197, 201]
    else:
        h1, m15 = _candles(np.linspace(200, 100, 220)), _candles(np.linspace(150, 100, 70))
...
def _config(selected, enabled=True):
    ...
    settings = {
        ...
        "MOMENTUM_CONTINUATION": {"min_rsi": 0, "max_rsi": 100, "min_ema_slope_atr": -100, "slope_lookback": 3},
        "BREAKOUT_CONTINUATION": {"breakout_lookback": 10, "min_rsi": 0, "max_rsi": 100, "min_breakout_atr": 0},
        "STRONG_TREND_CONTINUATION": {"min_adx": 0, "min_rsi": 0, "max_rsi": 100, "min_ema_slope_atr": -100, "slope_lookback": 3, "sustained_bars": 3},
    }[selected]
...
def test_each_entry_module_selects_buy_and_sell(module, direction):
    signal = TrendMomentumStrategy(multi_entry_config=_config(module)).evaluate("EURUSD", _market(direction))
...
def test_each_module_has_a_non_qualifying_case(module):
    ...
    signal = TrendMomentumStrategy(multi_entry_config=_config(module)).evaluate("EURUSD", flat)
    assert signal.action is SignalAction.HOLD
    assert signal.entry_details["module_results"][module]["status"] == "FAIL"


def test_disabled_module_cannot_trigger():
    signal = TrendMomentumStrategy(multi_entry_config=_config("EMA20_PULLBACK", False)).evaluate("EURUSD", _market("BUY"))
    assert signal.action is SignalAction.HOLD
    assert signal.entry_details["module_results"]["EMA20_PULLBACK"]["status"] == "DISABLED"


def test_priority_is_deterministic_and_only_one_signal_is_returned():
    cfg = _config("EMA20_PULLBACK")
    cfg["NEAR_EMA"] = {"enabled": True, "max_distance_atr": 100, "min_rsi": 0, "max_rsi": 100}
    signal = TrendMomentumStrategy(multi_entry_config=cfg).evaluate("EURUSD", _market("BUY"))
...
def test_h1_disagreement_blocks_all_modules():
    ...
    signal = TrendMomentumStrategy(multi_entry_config=_config("NEAR_EMA")).evaluate("EURUSD", data)
    assert signal.action is SignalAction.HOLD
