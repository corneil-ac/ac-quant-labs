...
def test_strategy_interface_and_registry():
    ...
    assert isinstance(
        strategy_registry.create("TrendMomentumStrategy"),
        TradingStrategy,
    )
...
def test_pullback_outside_lookback_does_not_trigger_entry():
    ...
    # Remove the recent EMA20 pullback from the test market.
    data["M15"].loc[67:, "low"] = data["M15"].loc[67:, "close"]

    # This is specifically an EMA20_PULLBACK regression test.
    # AQL-0044 introduced additional entry modules that may legitimately
    # produce BUY signals when the pullback module itself fails.
    # Disable those modules so this test continues to validate only
    # EMA20 pullback expiration behavior.
    ema_only = {
        "EMA20_PULLBACK": {
            "enabled": True,
            "pullback_lookback": 3,
...
def manager(
    ...
    broker.validate_volume.return_value = VolumeValidation(
        volume_ok,
        "EURUSD",
        0.01,
        0.01,
        ...
        1.0,
        0.01,
        "bad" if not volume_ok else "",
    )

    broker.buy.return_value = SimpleNamespace(
        ok=True,
        ticket=1,
    )

    risk = Mock()
    risk.can_open_symbol.return_value = (
        position_ok,
        "owned",
    )

    calendar = Mock()
    calendar.can_open_symbol.return_value = (
        calendar_ok,
        "news",
    )
    ...
    return (
        ExecutionManager(
            ...
            0.01,
            "entry",
            journal_path="/tmp/aql-test-journal.csv",
        ),
        broker,
        risk,
        calendar,
